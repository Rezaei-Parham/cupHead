...
public class MainBossController {
    ...
    private void moveOscillating(ActionEvent actionEvent) {
        ...
        if (boss.getHealth() <= 0) GamePane.getGamePane().endGame();
    ...
    private void changeImage(ActionEvent actionEvent) {
        if(boss == null) return;
        if (!boss.isShooting()) {
            boss.setPictureNumber((boss.getPictureNumber() + 1) % 6 + 1);
            ...
            if (boss.getPictureNumber() == 13) {
                ...
                boss.setPictureNumber(1);
                MusicsAddress.HOOHOO.playSong();
                boss.setX(600);
                return;
            }
            if (boss.getPictureNumber() == 8) MusicsAddress.GHUGHUL.playSong();
            if (boss.getPictureNumber() > 6) boss.setX(550);
            if (boss.getPictureNumber() == 3) MusicsAddress.BOSS_BARF.playSong();
    ...
    public void doDeathProcess() {
        ...
        deathAnimation.setCycleCount(27);
        boss.setPictureNumber(1);
        deathAnimation.play();
    ...
    private void setDieFrames(ActionEvent event) {
        ...
        if (boss.getPictureNumber() >= 27) {
            ...
            GamePane.getGamePane().removeEnemy(boss);
            GamePane.getGamePane().setBoss(new BabyBoss());
            deathAnimation.stop();
            GamePane.getGamePane().removeAnimation(deathAnimation);
            return;
        }
