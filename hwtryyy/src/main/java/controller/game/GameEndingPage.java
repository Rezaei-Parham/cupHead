...
public class GameEndingPage {
    ...
    public static int getScore() {
        if (Game.getGameMode().equals(Game.GameMode.NORMAL)) {
            if (Game.getDifficultyLevel().equals(GamePreData.LEVEL1))
                scoreNumber = (int) Math.ceil(100 - GamePane.getGamePane().getBoss().getHealth())
                        + (int) GamePane.getGamePane().getCupHead().getHealth();
            else if (Game.getDifficultyLevel().equals(GamePreData.LEVEL2))
                scoreNumber = (int) Math.ceil((100 - GamePane.getGamePane().getBoss().getHealth()) * 1.5)
                        + (int) GamePane.getGamePane().getCupHead().getHealth();
            else if (Game.getDifficultyLevel().equals(GamePreData.LEVEL3))
                scoreNumber = (int) Math.ceil((100 - GamePane.getGamePane().getBoss().getHealth()) * 2)
                        + (int) GamePane.getGamePane().getCupHead().getHealth();
        } else
            scoreNumber = (int) Math.ceil(100 - GamePane.getGamePane().getBoss().getHealth()) * 4
                    + (int) GamePane.getGamePane().getCupHead().getHealth();
        return scoreNumber;
    }
    ...
    public void initialize() {
        ...
        else { // pre setting the difficulty level
        ...
        pageChanger.play(); // changing the page transition
    ...
    private void setProgressLogo() {
        // set position by amount of lives left by boss
        double maxScore = 402;
        if (Game.getGameMode().equals(Game.GameMode.NORMAL)) {
